...
D' = \frac{D}{\mathrm{min}(p_1 (1-q_1), q_1(1-p_1) )}, \mathrm{\:for\:} D > 0
$$
...
`q1 * (1-p1)` is smaller, so we plug that into our $D'$ formula:
...
First we determine the denominator by calculating which of $p_1 (1-q_1)$ and $(1-p_1)q_1$ is smaller:
...
$(1-p1) * q1$ is smaller, so we use it for the denominator. $D'$ is:
