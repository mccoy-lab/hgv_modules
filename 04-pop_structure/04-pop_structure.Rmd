...
![**Fig. 1.** Two populations polymorphic for alleles `A` and `a`. The frequency of `A` in Population 1 is $0.83$, while its frequency in Population 2 is $0.15$.](04-pop_structure/images/pop_structure.jpg)
...
We're investigating population in genotype data from the [1000 Genomes Project](https://mccoy-lab.github.io/hgv_modules/the-1000-genomes-project.html) -- a sequencing dataset introduced in the genome browsers module.
...
Previewing `pop_labels` shows us that there are 26 "levels" in this vector, where each level is a population name.
...
`genetic_diff` outputs a table of $\textrm{G}_{ST}$ results, where every line corresponds to one variant from the input VCF. Our $\textrm{G}_{ST}$ values range from $0.009$ to $0.565$. 
...
![**Fig. 4.** A PCA plot that simplifies three-dimensional data into two dimensions.](04-pop_structure/images/pca.jpg)
