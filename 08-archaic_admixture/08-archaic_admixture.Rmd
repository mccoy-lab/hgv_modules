...
knitr::opts_knit$set(root.dir = '08-archaic_admixture')
...
3. Interpret the $D$ and $f_4$ statistics and their asssociated p-values.
...
```
```{r, echo = FALSE}
# read in table with the `d_result` data
# write.table(d_result, "d_all_Yoruba.txt",
#             sep = "\t", row.names = FALSE, quote = FALSE)
d_result <- read.table("d_French_Sardinian.txt", header = TRUE)
d_result
```
...
# write.table(d_result, "d_all_Yoruba.txt",
#             sep = "\t", row.names = FALSE, quote = FALSE)
d_result <- read.table("d_all_Yoruba.txt", header = TRUE)
d_result
```


***
<details> <summary> How do we interpret these $D$ results? </summary>
...
```
```{r, echo = FALSE}
# read in table with the `f4_result` data
# write.table(f4_result, "f4_all.txt",
#             sep = "\t", row.names = FALSE, quote = FALSE)
f4_result <- read.table("f4_all.txt", header = TRUE)
f4_result
```
...
```
```{r, echo = FALSE}
# read in table with the `f4_ratio_result` data
# write.table(f4_ratio_result, "f4_ratio_all.txt",
#             sep = "\t", row.names = FALSE, quote = FALSE)
f4_ratio_result <- read.table("f4_ratio_all.txt", header = TRUE)
f4_ratio_result
```
...
We've so far computed the $D$, $f_4$, and $f_4$-ratio statistics across the entire genome, but we can also restrict computation of these statistics to particular genomic intervals, such as genes, promoters, enhancers, etc.
...
Run the code below to re-calculate the $f_4$-ratio with `regions.bed`. We can create a new `snps` data object that either **keeps** or **excludes** these regions from our analysis.
...
Now we can re-calculate the $f_4$-ratio on one of the filtered SNP sets.
...
```
```{r, echo = FALSE}
# read in table with the `f4_filtered` data
# write.table(f4_filtered, "f4_filtered.txt",
#             sep = "\t", row.names = FALSE, quote = FALSE)
f4_filtered <- read.table("f4_filtered.txt", header = TRUE)
f4_filtered
```
...
<br></br>
* We also calculated the $f_4$ statistic, which is very similar to the $D$ statistic but is proportional to the genetic distance between two populations.
<br></br>
* Finally, we computed the $f_4$-ratio, whose value indicates the proportion of introgressed ancestry in each population.
  * We observed a higher proportion of Neanderthal ancestry in the Papuan individual in our dataset, which is likely an artifact of Denisovan introgression into Oceanian populations.
...
```
```{r, echo = FALSE}
# read in table with the `f4_keep` data
write.table(f4_keep, "f4_keep.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)
f4_keep <- read.table("f4_keep.txt", header = TRUE)
f4_keep
```
...
```
```{r, echo = FALSE}
# read in table with the `f4_remove` data
write.table(f4_remove, "f4_remove.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)
f4_remove <- read.table("f4_remove.txt", header = TRUE)
f4_remove
```
