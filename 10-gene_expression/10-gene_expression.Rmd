...
It's thought that many phenotypic differences within and between species trace back to differences in gene regulation, rather than in protein-coding sequences.
...
Now we'll look at the GTEx data ourselves to create our own visualizations of gene expression.
...
***
<details> <summary> Is there a sex-specific difference in _ACE2_ expression? </summary>

The p-value is significant (`p = 0.000194`), suggesting that _ACE2_ expression differs between males and females. Based on the coefficient of `-0.7907`, it looks like males tend to have ~0.8 fewer _ACE2_ transcripts in lung tissue.
...
Fit a linear model (or multiple!) to the GTEx data we downloaded in class and test for differential expression between two conditions. For example, you could look at another gene, compare between tissues, age groups, or Hardy classifications, etc. Based on the results, does your gene exhibit any significant difference in expression between conditions?