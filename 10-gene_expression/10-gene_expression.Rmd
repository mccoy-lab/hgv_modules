...
# file doesn't load in bookdown, so read it from google drive
read.table("https://drive.google.com/uc?export=download&id=1391X_47zTF4qh0URmmsqOwAifCZl8wKS", header = TRUE) %>%
  head()
```
...
# file doesn't load in bookdown, so read it from google drive
subset <- read.table("https://drive.google.com/uc?export=download&id=1FrLHrFRfvsOWKKOo6ZBAE8BbhZsb2iqa", header = TRUE)
head(subset)
```
...
```
```{r, echo = FALSE, message = FALSE}
# code block to preview hw file since loading it into bookdown won't work
# write.table(subset, "hw_ace2.txt", sep = "\t", quote = FALSE, row.names = FALSE)
subset <- read.table("data_for_rendering/hw_ace2.txt", header = TRUE)
head(subset)
```
