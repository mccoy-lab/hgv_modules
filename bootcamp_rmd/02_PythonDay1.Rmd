...
When we use the upper method on `pig`, we do not actually make a change to `pig` itself. To save the uppercase version of `pig`, we need to use variable assignment, for example as such:
...
Some methods operate differently. For example, the list `.append()` method which we will learn about *does* modify the underlying variable, so in the following code block: 
...
the line `myList.append('apple')` does actually modify `myList`, so there's no need to type something like `myList = myList.append('apple')` (this would in fact be an error). This is something you'll have to keep track of on a case-by-case basis as you learn new methods. 
