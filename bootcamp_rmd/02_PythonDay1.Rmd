...
To compute the standard deviation, we use the `np.std()`

### Max, min

We can get the maximum and minimum values of a `numpy` array using the `np.max()` and `np.min()` functions, respectively
...
This initializes a figure (`fig`) and a plot within the figure (`ax`). 
