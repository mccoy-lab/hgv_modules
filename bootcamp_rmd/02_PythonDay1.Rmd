...
* `int()`converts floats and strings into integers. Note that this function always rounds down if necessary. 
...
A special case of this is with `booleans`. Numeric values are **automatically converted** into `booleans`. `0` is equivalent to `False`. **All** non-zero numeric values are equivalent to `True`. Likewise, empty strings (`''`) convert to `False`. All non-empty strings (inlcuding the string `'False'`) convert to `True`.
